  ...
  <section id="about" class="section-padding relative overflow-hidden">
    ...
    <div ref="containerRef" class="max-w-6xl mx-auto px-6">
      ...
      <div class="text-center mb-16">
        ...
        <h2 :class="['text-4xl md:text-5xl font-extrabold mb-4', isDark ? 'text-white' : 'text-slate-900']">
          Building interfaces that
          <span class="text-gradient">matter.</span>
        </h2>
      </div>
      ...
      <div class="grid md:grid-cols-2 gap-16 items-center">
        ...
        <div class="reveal-left">
          ...
          <p :class="['text-lg leading-relaxed mb-10', isDark ? 'text-slate-400' : 'text-slate-600']">
            ...
            <span :class="['font-semibold', isDark ? 'text-white' : 'text-slate-900']">clean, scalable code</span> and
            <span :class="['font-semibold', isDark ? 'text-white' : 'text-slate-900']">component-based systems</span> that teams love.
          </p>

          <!-- Stats -->
          <div class="grid grid-cols-3 gap-4">
            <div v-for="(stat, i) in stats" :key="stat.label"
              :class="['reveal-flip glass rounded-2xl p-4 text-center card-hover neon-border', `delay-${i+1}`]">
              <div class="text-2xl font-black text-gradient mb-1">{{ stat.value }}</div>
              <div :class="['text-xs font-medium', isDark ? 'text-slate-500' : 'text-slate-500']">{{ stat.label }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="reveal-right relative">
          <div class="absolute inset-0 blur-3xl opacity-[0.15] rounded-3xl"
            style="background: linear-gradient(135deg, #6366f1, #8b5cf6, #06b6d4);"></div>

          <div class="relative glass rounded-3xl p-8 neon-border">
            <!-- Avatar icon -->
            <div class="relative w-20 h-20 mb-6">
              ...
              <div :class="['absolute -bottom-1 -right-1 w-5 h-5 bg-emerald-500 rounded-full border-2 flex items-center justify-center', isDark ? 'border-[#0a0d1a]' : 'border-white']">
                <span class="w-2 h-2 bg-emerald-300 rounded-full animate-ping"></span>
              </div>
            </div>
            ...
            <div class="space-y-3">
              <div v-for="item in profileItems" :key="item.text"
                :class="['flex items-center gap-3 text-sm group cursor-default', isDark ? 'text-slate-400' : 'text-slate-600']">
                <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 transition-all duration-300 group-hover:scale-110"
                  style="background: rgba(99,102,241,0.12); border: 1px solid rgba(99,102,241,0.2);">
                  <svg class="w-4 h-4 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="item.icon"/>
                  </svg>
                </div>
                <span :class="['transition-colors', isDark ? 'group-hover:text-slate-200' : 'group-hover:text-slate-900']">{{ item.text }}</span>
              </div>
            </div>

            <div class="mt-6 h-px shimmer rounded-full"></div>
            <div :class="['mt-4 flex items-center gap-2 text-xs', isDark ? 'text-slate-600' : 'text-slate-500']">
